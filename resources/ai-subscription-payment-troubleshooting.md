...
faq:
  - question: "AI 会员已经扣款但仍显示 Free，可以再买一次吗？"
    answer: "不要立即重复购买。先确认银行是否仍在处理付款、平台是否已经正式收款、订阅由哪个渠道管理、当前登录账号是否为原购买账号，以及官方产品页面是否已经显示会员。"
  ...
  - question: "应该联系银行、应用商店还是 AI 产品客服？"
    answer: "银行处理仍在处理中的银行卡交易、拒付和卡片限制；Apple、Google Play、X 或 Google One 处理各自渠道的账单；AI 产品客服处理已经收款但账号仍未显示会员的问题；ChongGrok 只处理 ChongGrok 订单。"
---
...
**先停止重复付款。**银行正在处理付款、商户已经收款、订阅显示有效和当前账号显示会员，是四个不同状态。正确顺序是：确认购买入口，核对原购买账号，检查订阅状态，最后再决定联系银行、账单平台、AI 产品支持或 ChongGrok 售后。
...
| 状态 | 能说明什么 | 下一步 |
| --- | --- | --- |
| 银行显示“待处理” | 银行记录了付款请求，但尚未确认款项最终付给商户 | 等待银行或商户给出最终结果，不重复付款 |
| 平台显示订单已完成或提供正式收据 | 账单方记录了已完成交易 | 查订阅由哪个渠道、哪个账号管理 |
| 订阅页面显示有效 | 账单系统存在有效计划 | 核对该计划绑定的 AI 产品账号 |
| 当前账号显示会员 | 当前账号已经识别付费计划 | 保存到期日与订单证据，完成验收 |
